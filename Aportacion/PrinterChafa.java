...
public class PrinterChafa<T extends Comparable<T>> {
    ...
    private void printNodoInternal(List<Nodo<T>> Nodos, int level, int maxLevel) {
        ...
        PrinterChafa.printWhitespaces(firstSpaces);
        ...
        for (Nodo<T> Nodo : Nodos) {
            ...
            PrinterChafa.printWhitespaces(betweenSpaces);
        }
        System.out.println("");

        for (int i = 1; i <= endgeLines; i++) {
            for (int j = 0; j < Nodos.size(); j++) {
                PrinterChafa.printWhitespaces(firstSpaces - i);
                if (Nodos.get(j) == null) {
                    PrinterChafa.printWhitespaces(endgeLines + endgeLines + i + 1);
                    continue;
                }
                ...
                if (Nodos.get(j).getIzquierda() != null)
                    ...
                    PrinterChafa.printWhitespaces(1);

                PrinterChafa.printWhitespaces(i + i - 1);
